...
type ListItem = { id: number; name: string; quantity: number; checked: number; price: number | null; imageUrl: string | null; productUrl: string | null; category: string; note: string; createdAt: string };
type Product = { id: string; title: string; price: number | null; image: string | null; link: string | null; rating: number | null; category: string };
...
const previewItems: ListItem[] = [
  { id: -1, name: "Bananas", quantity: 1, checked: 0, price: 0.58, imageUrl: null, productUrl: null, category: "Produce", note: "1 bunch", createdAt: "" },
  { id: -2, name: "Whole milk", quantity: 2, checked: 0, price: 3.16, imageUrl: null, productUrl: null, category: "Dairy", note: "Great Value, gallon", createdAt: "" },
  { id: -3, name: "Tortilla chips", quantity: 1, checked: 1, price: 2.74, imageUrl: null, productUrl: null, category: "Snacks", note: "", createdAt: "" },
];
...
export default function Home() {
  ...
  async function addItem(input: Partial<ListItem> & { name: string }) {
    ...
    const optimistic: ListItem = { id: optimisticId, name: input.name, quantity: input.quantity ?? 1, checked: 0, price: input.price ?? null, imageUrl: input.imageUrl ?? null, productUrl: input.productUrl ?? null, category: input.category ?? "", note: input.note ?? "", createdAt: new Date().toISOString() };
    ...
    <div className="mx-auto grid max-w-[1380px] gap-6 px-4 py-6 sm:px-8 lg:grid-cols-[minmax(0,1.1fr)_minmax(370px,.9fr)] lg:py-8">
      <section className="min-w-0">
        ...
        <div className="overflow-hidden rounded-3xl border border-border bg-card shadow-sm"><div className="flex items-center justify-between border-b border-border px-5 py-4"><p className="font-extrabold">To pick up</p><span className="rounded-full bg-[#e9f2ff] px-2.5 py-1 text-xs font-bold text-[#0756b6]">{remaining.length} {remaining.length === 1 ? "item" : "items"}</span></div>
          {loading ? <div className="space-y-3 p-5">{[0,1,2].map((key) => <Skeleton key={key} className="h-16 w-full rounded-2xl" />)}</div> : remaining.length === 0 ? <div className="grid min-h-52 place-items-center p-8 text-center"><div><span className="mx-auto mb-3 grid size-12 place-items-center rounded-full bg-[#e7f7ed] text-[#17743b]"><Check /></span><p className="font-extrabold">All picked up</p><p className="mt-1 text-sm text-muted-foreground">Add another item or search the Lincoln store.</p></div></div> : <GroupedItems items={remaining} updateItem={updateItem} removeItem={removeItem} />}
          {done.length > 0 && <details className="border-t border-border" open><summary className="flex cursor-pointer list-none items-center justify-between px-5 py-4 font-extrabold text-muted-foreground">Picked up <span className="flex items-center gap-2 text-sm font-bold">{done.length}<ChevronDown className="size-4" /></span></summary><div className="border-t border-border bg-muted/35"><GroupedItems items={done} updateItem={updateItem} removeItem={removeItem} /></div></details>}
        </div>
      </section>
...
function GroupedItems({ items, updateItem, removeItem }: { items: ListItem[]; updateItem: (id: number, changes: Partial<ListItem>) => void; removeItem: (id: number) => void }) {
  const groups = new Map<string, ListItem[]>();
  for (const item of items) {
    const category = item.category?.trim() || "Other";
    groups.set(category, [...(groups.get(category) ?? []), item]);
  }
  const categories = [...groups.keys()].sort((a, b) => a === "Other" ? 1 : b === "Other" ? -1 : a.localeCompare(b));
  return <div>{categories.map((category) => <section key={category}><div className="border-y border-border bg-[#f4f7fb] px-5 py-2 text-xs font-black uppercase tracking-[.12em] text-[#52667f] first:border-t-0">{category}</div><div className="divide-y divide-border">{groups.get(category)!.sort((a, b) => a.name.localeCompare(b.name)).map((item) => <ItemRow key={item.id} item={item} updateItem={updateItem} removeItem={removeItem} />)}</div></section>)}</div>;
}

function ProductCard({ product, addItem }: { product: Product; addItem: (item: Partial<ListItem> & { name: string }) => void }) {
  return <article className="flex gap-3 rounded-2xl bg-white p-3 text-slate-950 shadow-sm"><div className="grid size-20 shrink-0 place-items-center overflow-hidden rounded-xl bg-slate-100">{product.image ? <img src={product.image} alt="" className="h-full w-full object-contain mix-blend-multiply" /> : <ShoppingBasket className="size-7 text-slate-300" />}</div><div className="min-w-0 flex-1"><p className="line-clamp-2 text-sm font-extrabold leading-snug">{product.title}</p><p className="mt-1 truncate text-xs font-bold uppercase tracking-wide text-slate-500">{product.category || "Other"}</p><div className="mt-2 flex items-center justify-between gap-3"><div><p className="font-black text-[#0756b6]">{money(product.price)}</p>{product.rating != null && <p className="text-xs text-slate-500">★ {product.rating.toFixed(1)}</p>}</div><Button size="sm" onClick={() => addItem({ name: product.title, price: product.price, imageUrl: product.image, productUrl: product.link, category: product.category })} className="rounded-xl bg-[#0756b6] font-bold hover:bg-[#064b9f]"><Plus className="size-4" /> Add</Button></div></div></article>;
}
